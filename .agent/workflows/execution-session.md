...
- **Keep the project handoff set explicit**: in multi-MEU projects, `implementation-plan.md` and `task.md` must list the exact handoff path for each MEU so `/execution-critical-review` can load the full correlated review set instead of only the latest handoff
