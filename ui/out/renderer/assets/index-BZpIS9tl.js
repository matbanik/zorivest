...
const StatusBarContext = reactExports.createContext({
  message: "Ready",
  setStatus: () => {
  }
});
function StatusBarProvider(t0) {
  const $ = compilerRuntimeExports.c(6);
  const {
    children
  } = t0;
  const [message, setMessage] = reactExports.useState("Ready");
  const timerRef = reactExports.useRef(null);
  let t1;
  if ($[0] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t1 = (msg, t22) => {
      const durationMs = t22 === void 0 ? 3e3 : t22;
      if (timerRef.current) {
  ...
  let t3;
  if ($[3] !== children || $[4] !== t2) {
    t3 = /* @__PURE__ */ jsxRuntimeExports.jsx(StatusBarContext.Provider, { value: t2, children });
    $[3] = children;
    $[4] = t2;
    $[5] = t3;
  } else {
...
function useStatusBar() {
  return reactExports.useContext(StatusBarContext);
}
function StatusFooter() {
  ...
  const t0 = isReady ? "text-fg-muted" : "text-accent";
  let t1;
  if ($[0] !== message || $[1] !== t0) {
    t1 = /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "flex items-center px-4 py-1 text-xs border-t border-bg-elevated", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: t0, children: message }) });
    $[0] = message;
    $[1] = t0;
    $[2] = t1;
  } else {
...
async function fetchAllPreferences() {
  const entries = await Promise.all(SUPPRESSIBLE_CATEGORIES.map(async (cat) => {
    const key = `notification.${cat}.enabled`;
    try {
      const result = await apiFetch(`/api/v1/settings/${key}`);
      return [cat, result.value];
    } catch {
      return [cat, true];
    }
  }));
...
function NotificationProvider({
  ...
  const prefsRef = reactExports.useRef({});
  ...
  const notify = reactExports.useCallback(
    ({
      ...
      switch (category) {
        case "success":
          ue.success(message, toastOpts);
          break;
        case "warning":
          ue.warning(message, toastOpts);
          break;
        case "info":
        case "confirmation":
          ue.message(message, toastOpts);
          break;
      }
    },
    []
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(NotificationContext.Provider, { value: {
    notify,
    dismiss,
    preferencesLoaded: isFetched
  }, children: [
    children,
    /* @__PURE__ */ jsxRuntimeExports.jsx($e, { position: "bottom-right", toastOptions: {
