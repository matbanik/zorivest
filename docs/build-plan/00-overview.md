...
BUILD ORDER (left to right):
  ...
  │ Domain  │     │ Infrastructure│     │  Services   │     │ Entry Points│     │  Release  │
  │ + Ports │     │  (DB, Repos) │     │ (Use Cases) │     │API/MCP(TS)/ │     │EB/PyPI/npm│
  └─────────┘     └──────────────┘     └──────┬──────┘     │   GUI(TS)   │     └───────────┘
    0 deps          depends on 1         depends on 1,2    └─────────────┘       packaging
    pure Python     sqlcipher3           orchestration      depends on 3         distribution
    100% unit       integration tests    mock repos         REST → TS wrappers   installers
                                               │
                                               ▼
                                       ┌──────────────┐
                                       │   Phase 8    │
...
| 5 | [MCP Server](05-mcp-server.md) | Phase 4 | TypeScript MCP tools (calls REST) |
| 6 | [GUI](06-gui.md) | Phase 4 | Electron + React desktop app |
...
2. **The first line of code you write** is `test_calculator.py`. The first line of production code is `calculator.py`. Everything flows from there.
