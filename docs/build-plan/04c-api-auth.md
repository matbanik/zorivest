...
>
> [!NOTE]
> **Architecture change (MEU-42):** The MCP tool `get_confirmation_token` now generates tokens locally in the MCP layer and no longer calls this REST endpoint. This endpoint is retained for direct API consumers (e.g., delete_account, factory_reset).
...
# NOTE: MCP-layer destructive actions (zorivest_emergency_stop, create_trade,
# sync_broker, disconnect_market_provider, zorivest_service_restart) use
# MCP-local tokens generated by createConfirmationToken() in confirmation.ts.
# They do NOT call this REST endpoint. See 05j-mcp-discovery.md.
...
def test_create_confirmation_token(client):
    ...
    resp = client.post("/api/v1/confirmation-tokens", json={
        ...
        "params_summary": "Delete account DU3584717 and all associated data",
    })
    assert resp.status_code == 201
    data = resp.json()
    assert data["token"].startswith("ctk_")
    assert data["expires_in_seconds"] == 60
    assert data["action"] == "delete_account"
