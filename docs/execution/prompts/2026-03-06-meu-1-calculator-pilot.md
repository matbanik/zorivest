...
If `.\tools\validate.ps1` fails because unrelated repo surfaces are intentionally out of scope on day 1, do not widen scope to "fix the repo." Record the exact failure in the handoff under known risks / follow-up required and stop at `ready_for_review`.
