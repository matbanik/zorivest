...
This guide documents the complete release process for zorivest AI Commander maintainers.
...
2. **Download each platform executable**:
   - `zorivest-v1.0.0-windows.exe`
   - `zorivest-v1.0.0-linux.bin`
   - `zorivest-v1.0.0-macos-arm64.bin`
   - `checksums.txt`
...
- **Asset Naming**: `zorivest-{version}-{platform}{extension}`
...
zorivest uses a multi-layered versioning system to ensure version information is available across different installation methods (source, npm, PyPI, executables).

### Version Sources (Priority Order)

The application checks for version in this order:

1. **`zorivest/_version.py`** - Generated file containing exact version
...
| **npm package** | Contains `zorivest/_version.py` | `1.3.3` | Embedded in package |
...
2. Creates `zorivest/_version.py`:
   ```python
...
```bash
# Windows
set zorivest_VERSION_DEBUG=1
python zorivest.py

# Linux/macOS
zorivest_VERSION_DEBUG=1 python zorivest.py
```

Output shows which priority level succeeded:
```
[zorivest.version] Priority 1 success: _version.py -> 1.3.3
```
...
- ✅ Includes `zorivest/` directory in npm package (`package.json` `files` array)
- ✅ Bundles version module in PyInstaller executables (`zorivest.spec` `datas`)
...
- ❌ No clear benefit for zorivest's use cases
...
*This guide is maintained by the zorivest AI Commander development team.*
