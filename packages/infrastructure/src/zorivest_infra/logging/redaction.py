...
class RedactionFilter(logging.Filter):
    ...
    PATTERNS: list[tuple[re.Pattern[str], str]] = [
        ...
        (re.compile(r"ENC:[A-Za-z0-9+/=]+"), "[ENCRYPTED_VALUE]"),
        ...
        (
            re.compile(r"(X-[\w-]*(?:Key|Token|Auth)[\w-]*)\s*[:=]\s*\S+", re.I),
            r"\1: [REDACTED]",
        ),
        ...
        (re.compile(r"zrv_sk_[A-Za-z0-9_-]{32,}"), "[ZRV_KEY_REDACTED]"),
    ]
    ...
    def _redact_dict(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            else self._redact(str(v))
            if isinstance(v, str)
            else self._redact_dict(v)
