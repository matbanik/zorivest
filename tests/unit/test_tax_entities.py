...
class TestTaxLot:
    """MEU-123 AC-2/AC-3/AC-4: TaxLot entity with 11 stored fields + 2 computed."""
    ...
    def test_stored_field_count(self) -> None:
        """AC-2: TaxLot has exactly 11 stored fields (no computed in __init__)."""
        from zorivest_core.domain.entities import TaxLot
        import dataclasses

        init_fields = [f for f in dataclasses.fields(TaxLot)]
        assert len(init_fields) == 11
